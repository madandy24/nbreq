...
pub fn identity() -> Result<(Vec<u8>, Arc<ServerConfig>)> {
    ...
    root_params
        .distinguished_name
        .push(DnType::CommonName, "NBReq F5 ephemeral private CA");
...
pub fn run(args: &Args) -> Result<()> {
    ...
    let acceptor = thread::spawn(move || -> std::result::Result<(), String> {
        ...
        while !thread_stop.load(Ordering::Acquire) {
            ...
            if thread_stats.connections.fetch_add(1, Ordering::AcqRel) >= 256 {
                return Err("fixture connection bound exceeded".to_owned());
            }
            stream
                .set_nodelay(true)
            ...
            handlers.push(thread::spawn(move || {
                ...
                let _outcome = serve(connection, &stats, &stop, corrupt);
            }));
        }
    ...
    emit(json!({"event":"fixture_stopped", "stats":stats.json(), "joined":true}))?;
    Ok(())
}
