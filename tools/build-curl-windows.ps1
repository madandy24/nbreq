...
$artifactRoot = Join-Path $workspace "target\curl-pilot\windows-$Architecture"
...
if (-not $vsRoot) {
    throw 'Visual Studio with the x86/x64 C++ build tools was not found.'
}
...
if (-not (Test-Path -LiteralPath $archive)) {
    $alternateArchitecture = if ($Architecture -eq 'x64') { 'x86' } else { 'x64' }
    $alternateArchive = Join-Path $workspace "target\curl-pilot\windows-$alternateArchitecture\download\curl-$curlVersion.tar.xz"
    if (Test-Path -LiteralPath $alternateArchive -PathType Leaf) {
        Copy-Item -LiteralPath $alternateArchive -Destination $archive
    }
    else {
...
$status = @"
Package: curl
Architecture: $vcpkgTriplet
Version: $curlVersion
Status: install ok installed

"@
Set-Content -LiteralPath (Join-Path $statusRoot 'status') -Value $status -Encoding ascii
$manifest = @("$vcpkgTriplet/bin/libcurl.dll", "$vcpkgTriplet/lib/libcurl_imp.lib")
$manifest += Get-ChildItem -LiteralPath (Join-Path $tripletRoot 'include\curl') -File | ForEach-Object { "$vcpkgTriplet/include/curl/$($_.Name)" }
$manifest | Sort-Object | Set-Content -LiteralPath (Join-Path $statusRoot "info\curl_${curlVersion}_$vcpkgTriplet.list") -Encoding ascii

$dll = Join-Path $runtimeRoot 'bin\libcurl.dll'
$dllHash = (Get-FileHash -Algorithm SHA256 -LiteralPath $dll).Hash
Write-Host "Built curl $curlVersion ($Architecture, Schannel, HTTP/HTTPS, dynamic)"
