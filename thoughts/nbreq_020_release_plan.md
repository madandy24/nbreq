...
| Field | State |
| --- | --- |
| Candidate | Root 0.2.0, Rust 1.85 / edition 2024. Active branch `codex/pre-r5-release` extends completed R4 `68649ee`, including Wine fix `149450d`. Planning commit `db55c31` and failing-test commit `9c4c46d` precede the reservation fix and compatible runtime dependency ranges. [Pre-R5 report](nbreq_pre_r5.md) records the new candidate and validation. Main's mixed work remains separately owned. |
...
| Package inventory | Pre-R5 requires fresh archives for all three crates because their manifests changed. Earlier root/winpoll `149450d` and Darwin `4a6c807` packages remain historical evidence. New package path/hash inventories must come from the dedicated packaging directory; the stale main `target/package` archive is excluded. Registry-only resolution remains open. |
| Active slice | Pre-R5: three budget reds reproduced, all six budget tests green; complete Windows verifier 24/24 (131.476 s). Four fresh stable/MSRV consumers pass 34 stages, including formerly conflicting Mio 1.2.3. Clean packaging, evidence sealing and the remote branch checkpoint remain to finish this slice. |
...
| Next slice | Finish the agreed pre-R5 checkpoint, then run R5 against that integrated candidate. R4's four-host soaks and comparison evidence remain complete history. Separately authorize/publish Darwin 0.1.0 and winpoll 0.1.1, prove registry-only root installation, then publish/smoke 0.2.0. The owner-reported successful GDS pilot supports W-01 without blocking these library checks. |
