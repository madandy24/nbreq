...
Opened 2026-09-10; updated 2026-09-15. Status: R5 technical pass and owner's README/examples refresh complete; main integration, updated CI and final package/publication gates remain; R1 verified; R2 platform proof verified with local
...
| Field | State |
| --- | --- |
| Candidate | Root 0.2.0, Rust 1.85 / edition 2024. Active branch `codex/r5-release`; verified runtime source **`bdf5db5`** raises the compatible rustls minimum to patched 0.23.45. Presentation commit **`655a6e3`** adds the accepted 0.2 README and 17 grouped examples. [Housekeeping](nbreq_housekeeping.md) records main's preserved snapshot, tooling reconciliation and final integration checks; [R5 report](nbreq_r5_readiness.md) retains the earlier technical matrix. |
...
| Next slice | Complete authorized local [housekeeping](nbreq_housekeeping.md), final Windows checks and clean package freeze. Main's original 147 changed/untracked paths are preserved in snapshot **`cb18acc`**; integrate the reviewed R5 history by fast-forward. Updated hosted CI requires an approved public push including the new evidence archives. Then separately authorize/publish the helpers, prove registry-only root installation, and complete R6 root publication/smoke. The [examples](nbreq_examples_refresh.md) and [README](nbreq_readme_refresh.md) checkpoints retain their earlier validation. |
...
The tested code/workflow commit `bdf5db5` is pushed. The final R5 report and new 4,629,805-byte
evidence archive are prepared locally, but their public push was rejected by automatic approval
review because the earlier owner approval covered a different archive payload. Explicit approval
for this archive and the same public `madandy24/nbreq` destination is pending. The technical
checks are complete; the owner's README/examples discussion can proceed independently.
...
| R5 — Exact release candidate | Technical pass verified; documentation/package/registry gates open | Patched source bdf5db5 passes the hosted platform/stable/MSRV matrix, Windows x86 companion, root and fresh-consumer advisory checks, and locked license regeneration. [R5 evidence](nbreq_r5_readiness.md) retains the fixed advisory and unconfirmed initial x86 assertion. The owner-requested main-tree README/example refresh, affected example/doc checks, final normalized packages and links, and registry-only consumer proof remain before release acceptance. |
