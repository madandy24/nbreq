...
## WP9.5d streaming response state campaign — Windows slice

The second isolated target constructs a manual, socket-free held Engine and one unique response
reader for each input, then drives the production `StreamingResponseDecoder` directly. Eight
bounded control bytes select HTTP limits, a 1–64 byte reader queue, delivery versus discard,
fragment sizes, and reader-drain holes. Generated bytes therefore cross final-head arbitration,
body framing, queue-full suspension, passive reader release, EOF, terminal publication, and
discarded-response sink recovery without user code or network timing in the loop.

The driver asserts exact per-step consumption, forbids zero-progress and discard-backpressure
states, and requires a blocked decoder to expose bytes which immediately release capacity through
the real `ResponseReader`. Decode failures are committed to the real sink rather than hidden by
producer Drop. Five reviewed stable-suite seeds cover a one-byte fixed response window, chunking
and trailers through the same tiny window, immediate no-body completion, close-delimited EOF, and
a fully discarded redirect body.

The Windows target builds under the same isolated nightly/cargo-fuzz/libfuzzer-sys versions as the
buffered campaign. A 61-second run completes 348,758 executions at roughly 5,717 executions/second,
reaches 1,793 coverage points and 3,972 features, retains 702 corpus entries (about 76 KiB), and
produces no crash artifact. This accepts the first Windows streaming decoder/backpressure campaign;
DNS wire input, pool/socket transitions, Ubuntu repetition, and sustained pressure remain open.

The agreed longer “thrash” tier is deliberately separate from these one-minute development gates.
WP10 will run at least one hour of exact-source coverage-guided parser/state work on the Ubuntu
