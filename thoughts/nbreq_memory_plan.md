...
Opened: 2026-09-05 · Updated: 2026-09-15 · M0–M3 accepted; M4.1 verified; R1–R6 release gates complete: nbreq 0.2.0 and both helpers are published, with registry-only platform/MSRV proof. Device profiles remain separate.
...
| Next implementation step | NBReq 0.2.0 release is complete; [publication](nbreq_020_publication.md) records exact archives, tag, all-eight-job registry-only matrix and Windows x86 companion. Resume GDS workload/headroom measurements when available; M4/MQ-05 device profiles remain separate. [W-01](nbreq_wine_dns.md) retains actual Wine 5 x86/native evidence and the successful multi-day pilot report, whose formal deployment/shutdown provenance remains unrecorded. |
...
| Source identity | Published nbreq **0.2.0** and tag **`v0.2.0`** use **`d866179`**. Runtime checkpoint **`bdf5db5`** includes Wine discovery, the pre-R5 planned-body reservation fix and patched rustls minimum. Presentation **`655a6e3`** supplies README/examples; the final package adds one guide-link correction. Main snapshot **`cb18acc`** remains preserved. Publication changes no memory runtime behavior, GDS source or installed DLL. |
