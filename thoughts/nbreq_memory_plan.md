...
Opened: 2026-09-05 · Updated: 2026-09-15 · M0–M3 accepted; M4.1 verified; R4/R5 technical gates, release presentation and local main housekeeping complete. Updated hosted CI and publication gates are next; device profiles remain separate.
...
| Next implementation step | Follow the [release checklist](nbreq_020_release_plan.md) and [housekeeping checkpoint](nbreq_housekeeping.md) for approved main push, updated hosted CI and publication. Local main integration and fresh packages/Windows consumers pass at `432509b`; R4 soaks and the earlier R5 technical matrix are complete. Registry-only proof waits for helper publication. [W-01](nbreq_wine_dns.md) has actual Wine 5 x86 and native Windows evidence plus the owner's successful multi-day GDS pilot report; formal pilot provenance/shutdown details remain unrecorded. Resume GDS workload/headroom measurements when available; M4 device profiles are not a general-library release prerequisite. |
