...
```text
cargo +nightly fuzz run native_response_decoder tests/fixtures/fuzz/native_response_decoder -- -max_len=65536 -timeout=10
```

Run the streaming decoder/backpressure state machine similarly:

```text
cargo +nightly fuzz run native_streaming_response_decoder tests/fixtures/fuzz/native_streaming_response_decoder -- -max_len=65536 -timeout=10
```

The DNS wire/policy target is bounded to the resolver's 4 KiB packet ceiling:

```text
cargo +nightly fuzz run native_dns_response tests/fixtures/fuzz/native_dns_response -- -max_len=4100 -timeout=10
```
...
The shared corpus lives under `tests/fixtures/fuzz`, outside this separate Cargo package, so the
main crate archive contains the same seeds used by its compile-time regression tests. Cargo
excludes nested packages even when their files are listed in the parent's `include` patterns.
